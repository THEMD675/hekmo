...
export async function GET(request: NextRequest) {
  ...
  const state = Buffer.from(JSON.stringify({ businessId, userId: session.user.id })).toString("base64");
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    tokenUrl.searchParams.set("redirect_uri", `${process.env.NEXT_PUBLIC_APP_URL || "https://hekmo.ai"}/api/whatsapp/callback`);
