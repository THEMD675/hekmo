...
import { business, conversation, conversationMessage, businessKnowledge } from "@/lib/db/schema";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!businessRecord) {
      console.error("[WhatsApp] No business found for phone number ID:", businessPhoneNumberId);
      return NextResponse.json({ status: "ok" });
    }
    ...
    await sendWhatsAppMessage({
      ...
      to: customerPhone,
      message: aiResponse,
      accessToken: businessRecord.whatsappAccessToken || process.env.WHATSAPP_ACCESS_TOKEN!,
    });
    ...
    await db
      .update(business)
      .set({
        messagesThisMonth: ((businessRecord.messagesThisMonth as number) || 0) + 1,
        updatedAt: new Date(),
      })
...
async function generateBusinessAIResponse(
  ...
  try {
    ...
    const systemPrompt = buildBusinessPrompt(businessRecord, knowledgeContext, customerName);

    const { text } = await generateText({
      model: getLanguageModel("chat-model"),
      system: systemPrompt,
      messages: [
        ...(historyContext ? [{ role: "system" as const, content: `المحادثة السابقة:\n${historyContext}` }] : []),
