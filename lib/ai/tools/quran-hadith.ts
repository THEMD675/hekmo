...
export const quranHadithTool = tool({
  description:
    "Search the Quran and Hadith for Islamic guidance. Use when users ask about Quranic verses, Hadith, or Islamic rulings related to health, fasting, etc.",
  inputSchema: z.object({
    query: z.string().describe("Search query (can be in Arabic or English)"),
    source: z
      .enum(["quran", "hadith", "both"])
      .default("both")
      .describe("Which source to search"),
  ...
  execute: async ({ query, source, limit }) => {
    ...
    try {
      ...
      if (source === "quran" || source === "both") {
        ...
        if (quranResponse.ok) {
          ...
          if (quranData.data?.matches) {
            results.quran = quranData.data.matches
              .slice(0, limit)
              .map(
                (match: {
      ...
      return {
        ...
        results,
        suggestion:
          results.quran && results.quran.length > 0
            ? null
      ...
      return {
        ...
        query,
        suggestion:
          "يمكنك البحث في quran.com أو sunnah.com للحصول على نتائج دقيقة.",
      };
    }
  },
});
